...
var SHEET_URL = localStorage.getItem("customSheetUrl") || "https://docs.google.com/spreadsheets/d/1jfezjcRo8FOjUy15GZ_KG51SdUQo0AzD_TaoAM4QxlY/edit";
...
function renderEventSelect() {
  ...
  // ✅ عرض رابط المسح للمناسبة الحالية
  updateScanLink();
}

function updateScanLink() {
  var existingLink = document.getElementById("scanLinkBox");
...
function startScanner() {
  ...
  scanner.start({ facingMode: "environment" }, { fps: 30, qrbox: { width: 200, height: 200 } },
...
var isSelected = selectedGuests.has(guest.id);
row.innerHTML = '<td><input type="checkbox" class="guest-checkbox" data-id="' + guest.id + '" ' + (isSelected ? 'checked' : '') + ' style="width:18px;height:18px;cursor:pointer;" /></td><td><strong>' + escapeHtml(guest.name) + '</strong></td><td dir="ltr">' + escapeHtml(guest.phone) + '</td><td><span class="status-badge" style="background:' + (maxed ? '#fee2e2' : (guest.scanCount > 0 ? '#fef3c7' : '#f1f5f9')) + ';color:' + (maxed ? '#991b1b' : (guest.scanCount > 0 ? '#92400e' : '#475569')) + ';">' + (maxed ? '🚫 منتهي' : (guest.scanCount || 0) + '/' + (guest.maxScans || 1)) + '</span></td><td>' + (hasInv ? '<span style="color:#10b981;">✅ PDF</span>' : '<span style="color:#94a3b8;">—</span>') + '</td><td><button class="edit-btn"><i class="fas fa-edit"></i></button> <button class="delete-btn"><i class="fas fa-trash"></i></button> <button class="generate-single-btn" style="background:#dbeafe;color:#1e40af;border:none;border-radius:6px;padding:0.3rem 0.5rem;cursor:pointer;font-size:0.75rem;" title="توليد هذا الضيف فقط"><i class="fas fa-file-pdf"></i></button></td>';
